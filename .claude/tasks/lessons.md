...
- **GitHub Actions `if` conditions and `DEPLOY_STAGING` (2026-03-18)**: When adding a staging gate before production, don't carry forward the `vars.DEPLOY_STAGING != 'true'` guard on `deploy-infra`. The staging validation job itself is the gate — if it succeeds, production should proceed; if it's skipped (no staging), production should also proceed. The `always()` + result check pattern handles both cases. Adding the old `DEPLOY_STAGING` guard on top blocks production even after successful staging validation.