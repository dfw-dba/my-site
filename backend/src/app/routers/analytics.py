...
from src.app.schemas.analytics import AnalyticsEvent, PageViewData, VisitorEventData
...
async def record_analytics_event(
    ...
    if body.type == "page_view" and isinstance(body.data, PageViewData):
        data = body.data.model_dump()
        data["client_ip"] = client_ip
        data["is_bot"] = is_bot_request
        return await db.insert_page_view(data)
    elif body.type == "event" and isinstance(body.data, VisitorEventData):
        data = body.data.model_dump()
        return await db.insert_visitor_event(data)

    return {"success": False, "error": "Invalid event type/data combination"}
